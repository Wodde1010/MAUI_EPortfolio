...
namespace Example1
{
    class Snake : Field
    {
        ...
        public void move(char[,] template)
        {
            ...
            if (signWhereTheNewPartWillSpawn != signForTheBodyOfTheSnake && signWhereTheNewPartWillSpawn != signForTheFrame)
            {
                ...
                if (signWhereTheNewPartWillSpawn != signForFood)
                {
                    template[bodyInY[bodyInY.Count - 1], bodyInX[bodyInX.Count - 1]] = signForSpace;
